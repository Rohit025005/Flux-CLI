...
export async function clearStoredToken() {
  try {
    ...
    console.error("error in clearing the token:",error.message)
    return false;
  }
}
...
export async function loginAction(opts) {
    const options = z.object({
        severUrl : z.string().optional(),
        clientId : z.string().optional()
    }).parse(opts);

    const serverUrl = options.severUrl || DEMO_URL;
    const clientId = options.clientId || CLIENT_ID || process.env.GITHUB_CLIENT_ID; 

    intro(chalk.bold("🔐 Better Auth CLI Login"));

    if(!clientId){
        logger.error("CLIENT_ID is not in .env file");
        console.log(chalk.red("\n please get GITHUB_CLIENT_ID in ur .env file"));
        process.exit(1);
    }

    //check is already loged in
  ...
  try {
    ...
    spinner.stop();
    console.error(chalk.red("\nLogin failed (in catch):"), err.message);
    process.exit(1);
  }
